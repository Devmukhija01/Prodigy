...
router.post("/upload-avatar", requireAuth, upload.single('avatar'), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const avatarUrl = `https://prodigy-59mg.onrender.com/uploads/avatars/${req.file.filename}`;
...
router.post("/crop-avatar", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const newAvatarUrl = `https://prodigy-59mg.onrender.com/uploads/avatars/${newFilename}`;
